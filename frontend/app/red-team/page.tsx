...
const mockMetrics = {
  generation_rate_per_sec: 428,
  overall_success_rate: 0.065, // 6.5% bypass rate
  active_vectors: [
    { name: 'Velocity brute-force', load: 85, success_rate: 0.04 },
    { name: 'Synthetic Identity Injection', load: 45, success_rate: 0.09 },
    { name: 'Account Takeover (ATO)', load: 60, success_rate: 0.12 },
    { name: 'Distributed Card Testing', load: 95, success_rate: 0.02 },
  ]
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm flex flex-col justify-between h-48 border border-slate-200">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Simulated Load Rate</span>
            <Activity className="w-4 h-4 text-blue-600" />
          </div>
          <div className="flex items-end justify-between">
            <span className="text-5xl font-extrabold text-slate-900">{mockMetrics.generation_rate_per_sec} <span className="text-xl font-medium text-slate-400">req/s</span></span>
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm flex flex-col justify-between h-48 border border-slate-200">
          ...
          <div className="flex items-end justify-between">
            <span className="text-5xl font-extrabold text-rose-600">{(mockMetrics.overall_success_rate * 100).toFixed(1)}%</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 col-span-1 flex flex-col h-[28rem]">
          ...
          <div className="flex flex-col gap-6 flex-1">
            {mockMetrics.active_vectors.map((vector, idx) => (
              <div key={idx} className="space-y-3">
                <div className="flex justify-between items-end">
                  <span className="text-slate-900 font-semibold text-sm">{vector.name}</span>
                  <span className="text-sm text-slate-400">Yield: {(vector.success_rate * 100).toFixed(1)}%</span>
                </div>
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-blue-600 rounded-full" 
                    style={{ width: `${vector.load}%` }}
                  />
                </div>
              </div>
            ))}
